...
checkbox.addEventListener("change", function () {
  let unavailabilityStart = document.querySelector(
    ".unavailability-time-start"
  );
...
function loadUnavailabilityTableData() {
  ...
  if (current != null) {
    ...
    const table = document.querySelector(".unavailability-table");
    ...
    if (unavailabilityData.length !== 0) {
      ...
      unavailabilityData.forEach((unavailability) => {
        ...
        if (unavailability.allDay === "true") {
          ...
          startTime = new Date(unavailability.start).toLocaleTimeString(
            "en-us",
            options
          );
          endTime = new Date(unavailability.end).toLocaleTimeString(
            "en-us",
            options
          );
